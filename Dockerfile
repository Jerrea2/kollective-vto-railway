...
CMD ["/bin/bash","-lc","python /app/startup.py && CMD ["/bin/bash","-lc","uvicorn kollective_vto2.inference:app --host 0.0.0.0 --port 80 --log-level debug --proxy-headers --forwarded-allow-ips=\"*\"]
